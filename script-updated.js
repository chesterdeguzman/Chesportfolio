...
const projectDetails = {
  elearning: {
    ...
    repository: 'https://github.com/chesterdeguzman/REPLACE-WITH-REPOSITORY',
    medium: 'https://medium.com/@yosayosa/REPLACE-WITH-ARTICLE'
  },
  pharma: {
...
document.querySelectorAll('.project-detail-button').forEach(button => {
  button.addEventListener('click', () => {
    ...
    dialogContent.innerHTML = `
      ...
      <p>${project.impact}</p>
      ${(project.repository || project.medium) ? `
        <div class="dialog-link-group">
          ...
          ${project.medium ? `<a class="button button-secondary dialog-medium-button" href="${project.medium}" target="_blank" rel="noopener">Read Medium Article ↗</a>` : ''}
        </div>` : ''}
    `;
